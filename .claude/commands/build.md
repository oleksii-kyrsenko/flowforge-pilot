...
- Apply token additions from the spec to `tailwind.config` `theme.extend` AND append
  the rows to the Design token map in CLAUDE.md in the SAME branch (they must never
  diverge). No raw values in component code.
