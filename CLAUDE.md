...
- Slash commands: `/spec <ticket>`, `/build`, `/review`, `/ship`, `/design-fixes`.
...
- Every pipeline command (`/spec`, `/build`, `/review`, `/ship`, `/design-fixes`) MUST, as its first and last action, append a row to `docs/metrics/metrics.csv` (columns: `ticket,stage,event,timestamp_iso`; events: `start`/`done`) using `date -u +%Y-%m-%dT%H:%M:%SZ`. Never skip or backfill rows from memory.
