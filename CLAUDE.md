...
>
> Version: 3.95 · Date: 2026-07-16 · Owner: Frontend Developer (Next.js) · Language: EN (translated from RU v2.1)
...
- Every pipeline command (`/spec`, `/build`, `/review`, `/ship`, `/design-fixes`) MUST, as its first and last action, append a row to `docs/metrics/metrics.csv` (columns: `ticket,stage,event,timestamp_iso,clean_seconds,wait_seconds`; events: `start`/`pause`/`resume`/`done`) using `date -u +%Y-%m-%dT%H:%M:%SZ`. The last two columns are populated ONLY on the `done` row — computed from THIS run's own start/pause/resume rows for the same ticket+stage, already present in the file; left blank on `start`/`pause`/`resume` rows. Never estimate or backfill these two values from memory — they are pure arithmetic on timestamps already written earlier in this same run. Never skip or backfill any row from memory. **Additionally, at EVERY point where the command's own definition (above) requires it to STOP and wait for the human** (an approval checkpoint, a confirm-before-PR/transition gate, or an anomaly-preflight STOP) — append `event: pause` immediately before halting, and append `event: resume` as the very first action once that specific wait is resolved (the human's next explicit input unblocking it), before doing anything else. This is not a new checkpoint — just an additional timestamp around an existing one. Setup/batch commands (`/tickets`, `/baseline`) do NOT write metrics.csv; they record their activity in docs/progress-log.md. **`/design-fixes` writes metrics only when applying a designer reply (branch + PR); its `export` mode is a read-only render and does NOT write metrics.csv** — a pseudo-ticket row would pollute per-ticket cycle aggregation, like the `/tickets`/`/baseline` rows.
- **`clean_seconds`** (written on the `done` row) = `(done − start) − Σ(resume − pause)`, in whole seconds — the command's own active computation, excluding every human wait embedded inside its own execution (approvals, confirms, anomaly STOPs). **`wait_seconds`** (same row) = `Σ(resume − pause)`, in whole seconds. Both are computed and written by the command itself at the moment it appends the `done` row — never left as a manual exercise for whoever reads the file later. This is narrower than, and separate from, the cross-stage human intervals below (which measure the gap BETWEEN stages, e.g. spec done → build start) — pause/resume measures a wait INSIDE a single stage's own start→done span.
