...
### Design token map (generated at setup — GUIDE Prompt B fills this table after an approval checkpoint and applies the tokens to the Tailwind theme source [v4: `@theme` in the global stylesheet; v3: `theme.extend` in tailwind.config] via a PR)
...
| derived from styles — design face                                                                 | `--font-poppins`                                                                                | `"Poppins", …`                                                                                 |
...
- On spec approval, `/build` applies additions to the Tailwind theme source (v4: `@theme` in the global stylesheet; v3: `theme.extend` in tailwind.config) AND appends the map rows in the same PR — the map ↔ the theme source (here, the `@theme` block in `src/app/globals.css`) must never diverge; `/review` verifies. Raw values are forbidden in component code; each addition is logged in section 9.
