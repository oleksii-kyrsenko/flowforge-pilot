...
>
> Version: 4.02 · Date: 2026-07-20 · Owner: Frontend Developer (Next.js) · Language: EN (translated from RU v2.1)
...
Jira ticket + Figma mockup link
   → Analyst agent: reads the ticket (Atlassian MCP) and the mockup (Figma MCP),
     ...
     a11y requirements, acceptance criteria → written to docs/specs/<KEY>.md
     (Jira-comment posting of the full spec is deferred — see §4 ambition levels)
...
- **Deferred (optional — not required for MVP, Target, or Stretch):** `/spec` posting
  the full specification text as a Jira comment. Currently `docs/specs/<KEY>.md` is the
  sole spec artifact; the Jira ticket still receives its normal lifecycle status/assignee
  updates, just not the full spec text. Reintroduce only if stakeholder visibility of
  the spec content inside Jira itself becomes an actual need — not before.
...
Reusable UI components are built on two orthogonal prop axes, so BOTH the element's semantics and its look are prop-driven:

1. **Appearance** — a `variant` prop selects the visual treatment (e.g. `filled` / `outline` / …). One shared component serves a whole VISUAL family; a genuinely different look is its own component.
2. **Element polymorphism** — the component renders as whatever element the usage needs (native `button`, `a` for navigation, `input[type="submit"]`, etc.), selected via an `asChild`/`as` prop (Radix-style Slot), never by cloning the component per element. Accessibility must be correct for the ACTUALLY-rendered element (an `asChild` anchor is a real `<a>`; keyboard/focus semantics follow the rendered tag).

This is the default for every visual family across the project — buttons, links, CTAs, and equally non-interactive families (headings, text, …): similar-looking things share ONE polymorphic, variant-driven component; what it IS and how it LOOKS are decided by props. It is NOT a per-ticket requirement to restate — a ticket INHERITS this rule and a spec never re-specifies it. Which family a new element belongs to (reuse an existing component vs create a new one) is decided by the existing Reuse check (`/spec`), not by look-alike guessing: a divergent look → its own component; a shared look → the existing component gains a `variant`.
...
- **Node 0a — `/spec` (at start, right after the ticket lifecycle advance to In Progress):** update the base (`git fetch origin && git checkout dev && git pull`); create branch `feat/<JIRA-KEY>-<slug>` from up-to-date `dev` (hard rule 1) — or, if it already exists, check it out instead of creating a duplicate. This happens BEFORE the spec is drafted or approved — the branch exists as soon as the ticket itself is In Progress. No commit yet at this point.
- **Node 0b — `/spec` (on approval):** `git commit` the spec artifacts (`docs/specs/<JIRA-KEY>.md`, `docs/design-questions.md` if changed, the spec-stage `metrics.csv` rows) on the branch created at node 0a; **`git push -u origin feat/<JIRA-KEY>-<slug>` immediately** — makes the ticket branch and its spec commit visible to any developer, independent of who eventually runs `/build`. No PR opens here.
- **Node 1 — `/build`:** checkout the existing `feat/<JIRA-KEY>-<slug>` branch (created by `/spec`'s node 0a — the normal path, existing regardless of whether that /spec run's draft was ultimately approved) — `git fetch origin` first if it isn't local yet; if it does not exist (an edge case: a ticket whose `/spec` predates this rule, or a non-UI ticket), create it fresh from up-to-date `dev`. `git commit` the component code + unit tests (+ any `@theme` / `docs/design-tokens.md` token additions) **only after the green gate** (lint/typecheck/test:run), plus the build-stage `metrics.csv` rows. The `/spec` artifacts were already committed at node 0b — this commit adds only the build-stage content.
...
- **Push — `/spec` (immediate, node 0b) and `/ship` (node 3/4 onward):** `/spec` pushes once immediately on approval (above). `/ship` pushes the accrued build/review/ship commits (`git push -u origin <branch>`, **feature branch ONLY, never `dev`/`main`** — hard rule 1) and re-pushes as later commits accrue (the open PR auto-updates).
...
- Every pipeline command (`/spec`, `/build`, `/review`, `/ship`, `/design-fixes`) MUST, as its first and last action, append a row to `docs/metrics/metrics.csv` (columns: `ticket,stage,event,timestamp_iso,clean_duration,wait_duration,total_duration`; events: `start`/`pause`/`resume`/`done`/`total`) using `date -u +%Y-%m-%dT%H:%M:%SZ` for `timestamp_iso`. Setup/batch commands (`/tickets`, `/baseline`) do NOT write metrics.csv; they record their activity in docs/progress-log.md. **`/design-fixes` writes metrics only when applying a designer reply (branch + PR); its `export` mode is a read-only render and does NOT write metrics.csv.** **At EVERY point where the command's own definition (above) requires it to STOP and wait for the human** — append `event: pause` immediately before halting, and `event: resume` as the very first action once resolved.
- **Per-row duration (every `pause`/`resume`/`done` row, computed against the immediately preceding row of the SAME ticket+stage already present in the file — never estimated, never backfilled):** format `HH:MM:SS` (zero-padded; hours may exceed 24 — keep counting, never wrap to days). `start` leaves all three duration columns blank. Exactly ONE of `clean_duration`/`wait_duration` is filled per row (never `total_duration` on these rows):
  - `pause` or `done` closes an ACTIVE segment (since the last `start`/`resume`) → `clean_duration` = this row's timestamp − the preceding row's timestamp.
  - `resume` closes a WAIT segment (since the last `pause`) → `wait_duration` = this row's timestamp − the preceding row's timestamp.
- **Stage total (one row, appended by the command itself as the LAST thing it does, immediately after its own `done` row, same command invocation):** `<JIRA-KEY>,<stage>,total,<TS>,,,<TOTAL_DURATION>` — `<TS>` reuses the `done` row's own timestamp (no extra clock read); `clean_duration`/`wait_duration` blank; `<TOTAL_DURATION>` = the sum of every `clean_duration` + `wait_duration` value already written for this ticket+stage (i.e. the full elapsed wall time from `start` to `done` for that stage), formatted `HH:MM:SS`.
- **Task total (ONE row, appended ONLY by `/ship`, as the absolute LAST action of the entire cycle — after `/ship`'s own stage-total row above):** `<JIRA-KEY>,task,total,<TS>,,,<TASK_TOTAL_DURATION>` — `<TS>` reuses `/ship`'s own stage-total row's timestamp; `<TASK_TOTAL_DURATION>` = the sum of the `total_duration` values of every stage-total row already written for this ticket (`spec`+`build`+`review`+`ship`), formatted `HH:MM:SS`. This is the whole-task figure.
...
- **No-hint progress-log hygiene (engine-fix write-ups).** A `docs/progress-log.md` entry for an engine fix MUST describe the general rule and root-cause CLASS (what kind of gap, what the fixed rule now requires) — it must NOT narrate the specific ticket's specific incident (component names, the exact framing of what the draft said/decided) in a way a same-ticket rerun would read as canon before drafting. Reason: `/spec` and other commands read this file as part of canon, so a vividly-specific incident write-up handed straight back to the same ticket's next attempt turns a "does the fixed rule generalize" test into "can the agent repeat a hint it was just given" — not evidence the fix works. If a same-ticket rerun is expected, keep the entry mechanical/abstract; the ticket-specific narrative (if worth keeping at all) belongs in chat/session history, not in a file the next attempt's own context-gathering will read. A rerun whose correctness may be attributable to reading such an entry does not count as validating the fix, even if its own output is otherwise correct — note this plainly rather than silently crediting it.
