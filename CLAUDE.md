...
- On spec approval, `/build` applies additions to tailwind.config theme.extend AND appends the map rows in the same PR — the two must never diverge; `/review` verifies. Raw values are forbidden in component code; each addition is logged in section 9.
