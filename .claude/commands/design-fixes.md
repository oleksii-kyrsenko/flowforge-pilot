...
1. **FIRST**: append `<batch-label-or-JIRA-KEY>,design-fixes,start,<TS>` to
   `docs/metrics/metrics.csv` (columns per CLAUDE.md §11 "Metrics & logging"; the stage
...
2. **LAST**: (a) append the matching `done` row (same ticket + stage) —
   `<CLEAN_DURATION>` computed against the immediately preceding row, per CLAUDE.md §11's
   per-row duration rule (`HH:MM:SS`); (b) immediately append the stage-total row for
   this ticket+stage, reusing the same `<TS>`, per CLAUDE.md §11's stage-total rule.
   (No whole-task total here — that is `/ship`'s job, and design-fixes batches aren't
   always tied to a single ticket cycle.)
